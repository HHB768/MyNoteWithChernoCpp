...
#if 0  // use #if to control your program, it is useful when debugging
...
// your function will get a unique signature (p.s different naming styles in C and C++)
...
// the compiler will find if there are same definitions (twice or more) and report an error
// but a definition and a declaration is the same, it will be fine and this problem is found by the linker
...
// or you can set definition in .h to static or inline to let the file including it avoid error (will be introduced later)
